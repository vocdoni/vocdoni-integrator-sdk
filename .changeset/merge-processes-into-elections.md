...
**Nothing breaks.** `client.processes` is the very same instance as `client.elections`, and the exported `ProcessesCspClient` is an alias of `ElectionsClient` (`ProcessesCspClient === ElectionsClient`, so `instanceof` works either way). Both are deprecated and **will be removed in the next major version** — migrate by replacing `client.processes.` with `client.elections.`, which is a pure rename with no signature changes.

In `@vocdoni/api-voting`, `BlindCspApiClient` accepts a client keyed by either `elections` or the deprecated `processes`, so hand-rolled clients written against api-client 2.x keep working; the endpoint shape is now exported separately as `BlindCspEndpoints`. That fallback is removed in the next major too.

...
One behaviour change worth knowing about: the client now keeps its own snapshot of the config fields you pass in (including inherited properties) (so `setLang()` never reaches back and mutates yours). Mutating that object after construction — `config.authToken = next` — no longer affects the client; use the getter form for values that change.

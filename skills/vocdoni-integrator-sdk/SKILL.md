...
- **The OTP's language is the client's, not the UI's.** The 2FA code from `authStep0`/`resend` is rendered by the backend, so a voter reading a Catalan UI still gets an English SMS unless the client says otherwise. Pass `lang` to `new VocdoniApiClient({ apiUrl, lang })` — a string, or a getter like `() => i18n.language` that is re-read on every request — or call `client.setLang(...)` when the locale changes mid-session. It rides along as a `lang` query param on every request; leave it unset and the backend falls back on its own.

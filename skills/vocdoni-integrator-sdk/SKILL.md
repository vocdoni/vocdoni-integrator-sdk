...
- **Process status is computed — and a scheduled process is `UPCOMING`, not `ONGOING`.** `computeProcessStatus(questions, { startDate })` derives the top-level status from all question statuses. Any question `ONGOING` → `ONGOING`; all `ENDED`/`RESULTS` → `ENDED`. Statuses: `ONGOING`, `PAUSED`, `ENDED`, `CANCELED`, `UPCOMING`, `RESULTS`, `PROCESS_UNKNOWN`. The chain has no upcoming state: a published process with a future `startDate` reports its questions as live (`READY`) yet **rejects every vote until the start**, so always pass the process's `startDate` — live questions then read as `UPCOMING` until it passes. `isLive(process)` / `isUpcoming(process)` do this for you; `useElection().status` does too and flips to `ONGOING` on its own at the start.
...
- **A ranked question must declare itself; nothing else can tell.** A ranked `ballotProtocol` (`maxCount = numOptions`, `maxValue = numOptions - 1`, `uniqueValues: true`) is byte-identical to a pick-slot multichoice whose voters fill every slot, and the two are transposes — ranked reads the field index as the *option* and its value as the *rank*; pick-slot reads it as a *slot* holding a chosen option. Create one with `metadata: { type: { name: 'ranked' } }` alongside the raw protocol (the backend's `type` vocabulary is `singlechoice`/`multichoice` only, and it stores/echoes the metadata bag verbatim). Without it the question decodes as a multichoice and reports the same number for every option — integrator-sdk#22. Then: ranks go on the wire **highest = best** — pass the voter's ordering (best first) to `encodeQuestionSelections(question, order)`, which transposes it and encodes in one step; `rankedOrderToScores` does the transposition alone. A ranking must be complete, every choice needs a distinct `value`, `maxValue` must not be 0, and an election-level `ranked` declaration must have exactly one question — all four are refused at creation *and* at encode time. `decodeQuestionResults` aggregates **Borda** (`Σ count × rank`, so `votes` is points, not people) and emits **no abstain bucket**, and `react-components` renders a rank widget requiring a complete ranking. `declaresRanked(question)` reports whether a question is inferred ranked. More generally, **the protocol decides the ballot type and a declared name only breaks ties it can't resolve** (#58): a name the shape rules out is ignored — a `ranked` name on `maxCount: 1` is single-choice, an approval protocol labelled `single-choice-multiquestion` is approval — and only a question with no `ballotProtocol` is typed by name alone.
- **A question may carry no ballot type at all — infer it with `tryInferQuestionBallotType` in render code.** `GET /processes` also lists legacy vochain elections as read-only projections (`legacy: true`), and most of their questions arrive with `type: ""`, **no `ballotProtocol`** (despite the declared type) and no `metadata`. `inferQuestionBallotType(question)` throws on those by design ("cannot infer ballot type…") — there is nothing to infer from, and guessing would misread the results. Anything that runs during render must use `tryInferQuestionBallotType(question)` from `@vocdoni/ballot` instead, which returns `undefined` for such a question; treat `undefined` as "unknown": no ballot can be cast for it and its results cannot be decoded. `questionSelectionRange`, `questionReservesAbstain` and `decodeQuestionResults` still throw on it, so check first. `react-components` does this for you: the type badge and tip render nothing, `<ElectionQuestions />` shows the choices disabled with an `errors.question_unsupported` message and refuses to submit a ballot including it, and `<ElectionResults />` lists its choices with empty `votes`/`percent`.
...
- **Bearer credentials can be updated explicitly.** `client.setAuthToken(token)` accepts a string or sync/async getter for subsequent requests; `client.setAuthToken()` clears the default Authorization header. The constructor snapshots config fields, so use setters or function-valued options instead of mutating the original object. This is separate from the voter's CSP token in request payloads.
- **The client can override the OTP language.** Without an explicit `lang`, the voter receives the OTP email/SMS in the organization's default language. Pass `lang` to `new VocdoniApiClient({ apiUrl, lang })` — a string, or a getter like `() => i18n.language` that is re-read on every request — or call `client.setLang(...)` when the locale changes mid-session. It rides along as a `lang` query param on every request; leave it unset to use the organization's default language.
